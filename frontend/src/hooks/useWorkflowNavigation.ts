...
export const useWorkflowNavigation = () => {
  ...
  const WORKFLOWS = {
    insertion: ["/treatment/select", "/treatment/scan", "/treatment/list"],
    removal: ["/treatment/select", "/treatment/removal"],
  } as const;
  ...
  const navigateBack = () => {
    const currentType = new URLSearchParams(location.search).get("type") as
      | "insertion"
      | "removal"
      | null;
    const typeToUse = currentType || procedureType || "insertion";
    const workflow =
      WORKFLOWS[typeToUse as keyof typeof WORKFLOWS] || WORKFLOWS.insertion;
  ...
  const getCurrentStepInfo = () => {
    ...
    const currentIndex = workflow.indexOf(location.pathname as any);
    const currentType =
      new URLSearchParams(location.search).get("type") || procedureType;
