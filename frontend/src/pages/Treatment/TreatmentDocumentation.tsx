...
import {
  isValidOfflineStatusTransition,
  validateOfflineScan,
  TreatmentType,
} from "@/services/offlineValidationService";
import { useTreatment } from "@/context/TreatmentContext";
import { useAuth } from "@/context/AuthContext";
import { useOffline } from "@/context/OfflineContext";
import applicatorService, {
  ApplicatorValidationResult,
} from "@/services/applicatorService";
...
const TreatmentDocumentation = () => {
  ...
  const [pendingValidation, setPendingValidation] =
    useState<ApplicatorValidationResult | null>(null);
  const [showApplicatorList, setShowApplicatorList] = useState(true);
  // Offline confirmation dialog state
  const [showOfflineConfirmDialog, setShowOfflineConfirmDialog] =
    useState(false);
  const [offlinePendingApplicator, setOfflinePendingApplicator] =
    useState<any>(null);
  const [searchSuggestions, setSearchSuggestions] = useState<any[]>([]);
  const [showSuggestions, setShowSuggestions] = useState(false);
  const [aSuffixQuery, setASuffixQuery] = useState("");
  const [showFinalizeConfirmDialog, setShowFinalizeConfirmDialog] =
    useState(false);
  ...
  const [originalApplicatorStatus, setOriginalApplicatorStatus] = useState<
    string | null
  >(null);
  ...
  const [validatedSeedLength, setValidatedSeedLength] = useState<number | null>(
    null,
  );
  ...
  const [formData, setFormData] = useState({
    ...
    usingType: "", // Legacy field - used as fallback when status is not set
    status: "", // 8-state workflow status (preferred over usingType)
    insertedSeedsQty: "",
    comments: "",
  });
  ...
  const getTreatmentType = (): TreatmentType => {
    ...
    if (
      site.includes("pancreas") ||
      site.includes("prostate") ||
      type.includes("pancreas") ||
      type.includes("prostate")
    ) {
      return "panc_pros";
    }
  ...
  const effectiveCurrentStatus =
    isEditMode && currentApplicator?.status
  ...
  const shouldShowStatus = (status: string): boolean => {
    ...
    return (
      ...
      allowedStatuses.includes(status as ApplicatorStatus)
    );
  };
  ...
  useEffect(() => {
    const loadOfflineDataIfNeeded = async () => {
      ...
      if (
        !isOnline &&
        currentTreatment?.id &&
        isDownloaded(currentTreatment.id)
      ) {
        try {
          ...
          setError(
            "Failed to load offline data. Please reconnect to continue.",
          );
        }
      }
    };
    ...
    if (isEditMode && currentApplicator) {
      const usageTypeLabel =
        currentApplicator.usageType === "full"
          ? "Full use"
          : currentApplicator.usageType === "faulty"
            ? "Faulty"
            : currentApplicator.usageType === "none"
              ? "No Use"
              : "";
      ...
      setFormData({
        ...
        insertionTime:
          currentApplicator.insertionTime || new Date().toISOString(),
  ...
  const addOfflineApplicatorsToContext = (
    offlineApplicators: OfflineApplicator[],
  ) => {
  ...
  const loadAvailableApplicators = async () => {
    ...
    if (!isOnline && isDownloaded(currentTreatment.id)) {
      try {
        const offlineApplicators = await offlineDb.getApplicatorsByTreatment(
          currentTreatment.id,
        );
  ...
  useEffect(() => {
    ...
    switch (currentStatus) {
      ...
      case "INSERTED":
        ...
        setFormData((prev) => ({
          ...prev,
          insertedSeedsQty: seedsQty.toString(),
        }));
        break;
      case "FAULTY":
  ...
  const handleBarcodeScanned = async (serialNumber: string) => {
    ...
    try {
      ...
      if (!isOnline && isDownloaded(currentTreatment.id)) {
        const offlineValidation = await validateOfflineScan(
          serialNumber,
          currentTreatment.id,
        );
        ...
        const offlineApplicator =
          await offlineDb.getApplicatorBySerial(serialNumber);

        if (offlineApplicator) {
          // Fill form with offline applicator data
          fillFormWithOfflineApplicatorData(offlineApplicator);
          setSuccess(
            `Applicator ${serialNumber} loaded from offline storage. Status: ${offlineApplicator.status || "SEALED"}`,
          );
        } else {
          setError(
            "Applicator not found in offline storage. Only pre-downloaded applicators are available offline.",
          );
        }
        setLoading(false);
        return;
      ...
      if (isDownloaded(currentTreatment.id)) {
        try {
          ...
          if (offlineApplicator) {
            fillFormWithOfflineApplicatorData(offlineApplicator);
            setSuccess(
              `Applicator ${serialNumber} loaded from offline cache (server unavailable).`,
            );
            setLoading(false);
            return;
          }
  ...
  const fillFormWithApplicatorData = async (
    validation: ApplicatorValidationResult,
  ) => {
    ...
    setSuccess(
      `Applicator ${serialNumber} validated successfully! Status set to SEALED - select appropriate status below.`,
    );
  };
    ...
    setFormData({
      ...
      insertedSeedsQty:
        currentStatus === "SEALED"
          ? "0"
          : applicator.insertedSeedsQty?.toString() || "0",
      comments: applicator.comments || "",
    });

    if (currentStatus !== "SEALED") {
      setSuccess(
        `Applicator ${applicator.serialNumber} selected successfully! Current status: ${currentStatus} - select next status below.`,
      );
    } else {
      setSuccess(
        `Applicator ${applicator.serialNumber} selected successfully! Status set to SEALED - select appropriate status below.`,
      );
    }
  };
  ...
  const adjustTime = (minutes: number) => {
    ...
    const newTime =
      minutes > 0
        ...
        : subMinutes(currentTime, Math.abs(minutes));
    setFormData((prev) => ({
      ...prev,
  ...
  const handleNext = async () => {
    ...
    if (
      (requiresComment(currentStatus) || currentStatus === "Faulty") &&
      (!formData.comments || formData.comments.trim().length === 0)
    ) {
      setError(
        "Comments are required for this status. Please explain why the applicator was not used successfully.",
      );
      return;
    }
    ...
    if (!isOnline) {
      ...
      const previousStatus =
        originalApplicatorStatus as ApplicatorStatus | null;
      const validation = isValidOfflineStatusTransition(
        previousStatus,
        currentStatus as ApplicatorStatus,
        getTreatmentType(),
      );
      ...
      try {
        ...
        await processApplicatorOffline(
          applicatorData,
          false,
          originalApplicatorStatus as ApplicatorStatus | null,
        );
        ...
        setSuccess(
          "Applicator status saved offline. Will sync when connection is restored.",
        );
        setError(null);
      } catch (err: any) {
    ...
    try {
      ...
      const saveResult = await applicatorService.saveApplicatorData(
        currentTreatment.id,
        applicatorData,
      );
      ...
      let uploadSyncStatus: "pending" | "syncing" | "synced" | "failed" | null =
        null;
      ...
      if (selectedFiles.length > 0 && savedApplicatorId) {
        ...
        if (!uploadResult.success) {
          ...
          setError(
            `Applicator saved, but file upload failed: ${uploadResult.message}`,
          );
          ...
          uploadSyncStatus =
            (uploadResult.syncStatus as "synced" | "failed") || "synced";
      ...
      const applicator = {
        ...
        catalog:
          saveResult.applicator?.catalog || validatedCatalog || undefined,
        seedLength:
          saveResult.applicator?.seedLength || validatedSeedLength || undefined,
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-sm text-yellow-700">
                  ...
                  restored. Medical status changes (INSERTED/FAULTY) require
                  confirmation.
                </p>
              </div>
            </div>
              ...
              {currentTreatment ? (
                <div className="space-y-3 text-sm">
                  ...
                  <div>
                    ...
                    <p className="font-medium">
                      {(() => {
                        const currentSite = user?.sites?.find(
                          (site) =>
                            typeof site === "object" &&
                            site.custName === currentTreatment.site,
                        );
          ...
          <div className="space-y-4 md:space-y-6 md:col-span-1 lg:col-span-2">
            ...
            <div className="rounded-lg border bg-white p-4 shadow-sm">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-lg font-medium">
                  {isEditMode
                    ? "Edit Applicator Details"
                    : manualEntry
                      ? "Enter Serial Number Manually"
                      : "Scan Applicator Barcode"}
                </h2>
                {!isEditMode && (
                  <button
              ...
              {isEditMode ? (
                <div className="rounded-md bg-gray-50 border border-gray-300 p-4">
                  <p className="text-sm text-gray-700 flex items-center gap-2">
                    ...
                    Serial number cannot be changed when editing an existing
                    applicator. You can modify other fields below.
                  </p>
                </div>
              ) : manualEntry ? (
                            ...
                            <div className="space-y-1 p-2">
                              {aSuffixQuery && (
                                <div className="p-2 border-b bg-primary/10 text-sm font-medium text-primary">
                                  {filteredApplicators.length} applicator(s){" "}
                                  {aSuffixQuery
                                    ? `ending with "-A${aSuffixQuery}"`
                                    : "available"}
                                </div>
                              )}
                              {filteredApplicators
                                .sort((a, b) => {
                                  ...
                                  const priorityA =
                                    statusPriority[statusA] ?? 2;
                                  const priorityB =
                                    statusPriority[statusB] ?? 2;
                                ...
                                .map((applicator, index) => {
                                  const applicatorStatus =
                                    applicator.status || "SEALED";
                                  const statusColor =
                                    getListItemColor(applicatorStatus);
                                  const statusEmoji =
                                    getStatusEmoji(applicatorStatus);
                                  const statusLabel =
                                    getStatusLabel(applicatorStatus);

                                  return (
                                    <button
                                      key={index}
                                      onClick={() =>
                                        handleApplicatorSelect(applicator)
                                      }
                                    ...
                                    >
                                        ...
                                        {applicatorStatus !== "SEALED" && (
                                          <span
                                            className={`text-xs px-2 py-0.5 rounded ${
                                              applicatorStatus === "OPENED"
                                                ? "bg-red-200 text-red-800"
                                                : applicatorStatus === "LOADED"
                                      ...
                                      <div className="text-sm text-gray-500">
                                        ...
                                        {applicator.seedQuantity} sources
                                      </div>
                                    </button>
                                  );
                                })}
                          ...
                          <div className="space-y-1 p-2">
                            {searchSuggestions.map((suggestion, index) => {
                              const suggestionStatus =
                                suggestion.status || "SEALED";
                              const statusEmoji =
                                getStatusEmoji(suggestionStatus);
                              const statusLabel =
                                getStatusLabel(suggestionStatus);
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <div className="text-gray-600">
                                    {suggestion.applicatorType} •{" "}
                                    {suggestion.seedQuantity} sources
                                  </div>
                                </button>
                              );
                            })}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <input
                        type="text"
                        value={format(
                          new Date(formData.insertionTime),
                          "dd.MM.yyyy HH:mm",
                        )}
                        readOnly
                  ...
                  <div>
                    ...
                    >
                      ...
                      {shouldShowStatus("SEALED") && (
                        <option value="SEALED" className="text-gray-800">
                          Sealed (unopened)
                        </option>
                      )}
                      {shouldShowStatus("OPENED") && (
                        <option value="OPENED" className="text-red-600">
                          Opened (package opened)
                        </option>
                      )}
                      {shouldShowStatus("LOADED") && (
                        <option value="LOADED" className="text-yellow-600">
                          Loaded (ready for insertion)
                        </option>
                      )}
                      {shouldShowStatus("INSERTED") && (
                        <option value="INSERTED" className="text-green-600">
                          Inserted (successfully deployed)
                        </option>
                      )}
                      {shouldShowStatus("FAULTY") && (
                        <option value="FAULTY" className="text-gray-800">
                          Faulty (defective equipment)
                        </option>
                      )}
                      {shouldShowStatus("DISPOSED") && (
                        <option value="DISPOSED" className="text-gray-800">
                          Disposed (discarded)
                        </option>
                      )}
                      {shouldShowStatus("DISCHARGED") && (
                        <option value="DISCHARGED" className="text-gray-800">
                          Discharged (sources expelled)
                        </option>
                      )}
                      {shouldShowStatus("DEPLOYMENT_FAILURE") && (
                        <option
                    ...
                    formData.usingType === "Faulty" ? (
                      <input
                        ...
                        onChange={(e) =>
                          setFormData((prev) => ({
                            ...prev,
                            insertedSeedsQty: e.target.value,
                          }))
                        }
                        className="block w-full rounded-md border border-gray-300 px-3 py-2 shadow-sm focus:border-primary focus:outline-none focus:ring-primary text-base md:text-sm min-h-[44px]"
                        disabled={loading}
                  ...
                  <div className="md:col-span-2">
                    ...
                    {currentApplicator?.attachmentFileCount &&
                      currentApplicator.attachmentFileCount > 0 && (
                        <div className="mt-2 text-sm text-gray-600 flex items-center gap-1">
                          <span>📁</span>
                          <span>
                            {currentApplicator.attachmentFileCount} file(s)
                            already attached
                          </span>
                        </div>
                      )}
            ...
            <div className="rounded-lg border bg-gray-50 p-4">
              ...
              <ul className="text-xs text-gray-600 space-y-1">
                ...
                <li>
                  • System validates applicator against Priority database in
                  real-time
                </li>
                <li>• Insertion time is automatically set when scanning</li>
                <li>
                  • For 'Faulty' usage type, specify actual inserted sources
                  quantity
                </li>
                <li>
                  • Use 'Next' to add another applicator or 'Finalize' to
                  complete treatment
                </li>
              </ul>
            </div>
      ...
      <OfflineInsertionConfirmDialog
        ...
        onConfirm={async () => {
          if (offlinePendingApplicator) {
            ...
            setOriginalApplicatorStatus(null);
            setSuccess(
              "Applicator processed offline. Will sync when connection is restored.",
            );
          }
        }}
        applicatorSerial={offlinePendingApplicator?.serialNumber || ""}
        status={
          offlinePendingApplicator?.status === "FAULTY" ? "FAULTY" : "INSERTED"
        }
      />
    </Layout>
  );
