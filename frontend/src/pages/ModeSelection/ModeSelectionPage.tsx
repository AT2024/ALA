...
const ModeSelectionPage = () => {
  ...
  const handleProceedClick = async () => {
    ...
    try {
      ...
      const response = await api.put("/admin/test-mode", {
        enabled: isTestMode,
      });
    ...
    <Layout title="Select Mode" showBackButton={false}>
      <div className="mx-auto max-w-md space-y-6">
        <div className="rounded-lg border bg-white p-6 shadow-md">
          <h2 className="mb-2 text-xl font-medium text-center">
          ...
          {selectedMode === "test" && (
            <div className="mt-4 rounded-md bg-orange-100 p-3 text-sm text-orange-800">
              <strong>Test Mode:</strong> All data will be simulated. No real
              Priority API calls will be made. An orange banner will be visible
              at all times.
            </div>
          )}
