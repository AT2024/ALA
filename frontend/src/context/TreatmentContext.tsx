...
export function TreatmentProvider({ children }: { children: ReactNode }) {
  ...
  const [currentTreatment, setCurrentTreatment] = useState<Treatment | null>(
    () => {
      ...
      try {
        ...
        console.error(
          "Failed to restore treatment from sessionStorage:",
          error,
        );
        return null;
      }
  ...
  const [availableApplicators, setAvailableApplicators] = useState<
    Applicator[]
  >(() => {
    ...
    try {
      ...
      console.error(
        "Failed to restore available applicators from sessionStorage:",
        error,
      );
      return [];
    }
  ...
  const [processedApplicators, setProcessedApplicators] = useState<
    Applicator[]
  >(() => {
    ...
    try {
      ...
      console.error(
        "Failed to restore processed applicators from sessionStorage:",
        error,
      );
      return [];
    }
  ...
  const [currentApplicator, setCurrentApplicator] = useState<Applicator | null>(
    null,
  );
  ...
  const [individualSeedsRemoved, setIndividualSeedsRemoved] = useState<number>(
    () => {
      ...
      try {
        ...
        console.error(
          "Failed to restore individual seeds removed from sessionStorage:",
          error,
        );
        return 0;
      }
  ...
  useEffect(() => {
    try {
      ...
      sessionStorage.setItem(
        ...
        JSON.stringify(processedApplicators),
      );
      sessionStorage.setItem(
        "availableApplicators",
        JSON.stringify(availableApplicators),
      );
      sessionStorage.setItem(
        "individualSeedsRemoved",
        JSON.stringify(individualSeedsRemoved),
      );
    } catch (error) {
  ...
  const addAvailableApplicator = (applicator: Applicator) => {
    setAvailableApplicators((prev) => {
      ...
      const exists = prev.some(
        (app) => app.serialNumber === applicator.serialNumber,
      );
      if (exists) {
  ...
  const getFilteredAvailableApplicators = (): Applicator[] => {
    ...
    return availableApplicators.filter(
      (app) =>
  ...
  const getApplicatorProgress = () => {
    ...
    const total = currentTreatment?.seedQuantity
      ? Math.ceil(currentTreatment.seedQuantity / 25)
      : used; // Estimate based on 25 seeds per applicator
    return { used, total };
  };

  const getSeedProgress = () => {
    const inserted = applicators.reduce(
      (sum, app) => sum + (app.insertedSeedsQty || 0),
      0,
    );
    ...
    processedApplicators.forEach((app) => {
      ...
      switch (status) {
        ...
        case "DISPOSED":
          ...
          break;
        case "DISCHARGED":
          distribution.discharged++;
          break;
        case "DEPLOYMENT_FAILURE":
          distribution.deploymentFailure++;
          break;
        default:
          distribution.sealed++;
          distribution.none++;
  ...
  const getRemovalProgress = (): RemovalProgress => {
    ...
    const removedFromApplicators = applicators.reduce(
      (sum, app) => (app.isRemoved ? sum + app.seedQuantity : sum),
      0,
    );

    const effectiveTotalSeeds = currentTreatment?.seedQuantity || totalSeeds;
    const effectiveRemovedSeeds =
      removedFromApplicators + individualSeedsRemoved;
   ...
   */
  const canProcessOffline = useCallback(
    async (
      applicator: Applicator,
      newStatus: ApplicatorStatus,
      previousStatus?: ApplicatorStatus | null,
      ...
      const result = isValidOfflineStatusTransition(
        currentStatus,
        newStatus,
        getTreatmentType(),
      );
   ...
   */
  const processApplicatorOffline = useCallback(
    async (
      applicator: Applicator,
      ...
      const validation = await canProcessOffline(
        applicator,
        newStatus,
        originalStatus,
      );
      ...
      try {
        ...
        const changeHash = await crypto.subtle
          .digest(
            "SHA-256",
        ...
        const pendingChange: Omit<PendingChange, "id"> = {
          ...
          status: "pending",
          offlineSince:
            networkStatus.offlineStartTime?.toISOString() ||
  ...
  const loadFromOfflineDb = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      const offlineTreatment = await offlineDb.getTreatment(
        currentTreatment.id,
      );
      if (!offlineTreatment) {
        return;
      ...
      setAvailableApplicators((prev) => {
        ...
        localApplicators
          .filter(
            (a) => !TERMINAL_STATUSES.includes(a.status as ApplicatorStatus),
          )
      ...
      setProcessedApplicators((prev) => {
        ...
        localApplicators
          .filter((a) =>
            TERMINAL_STATUSES.includes(a.status as ApplicatorStatus),
          )
          .forEach((a) => applicatorMap.set(a.serialNumber, a));
        return Array.from(applicatorMap.values());
      });
  ...
  const actuallyUsedApplicators = processedApplicators.filter(
    (app) => app.usageType !== "none",
  );

  const progressStats: ProgressStats = {
    totalApplicators:
      actualAvailableApplicators.length + actuallyUsedApplicators.length,
    usedApplicators: actuallyUsedApplicators.length,
    totalSeeds: actualTotalSeeds,
    insertedSeeds: actualInsertedSeeds,
    completionPercentage:
      actualTotalSeeds > 0
        ? Math.round((actualInsertedSeeds / actualTotalSeeds) * 100)
        : 0,
