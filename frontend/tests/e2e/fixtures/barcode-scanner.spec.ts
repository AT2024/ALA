...
test.describe("Barcode Scanner Functionality", () => {
  ...
  test("should simulate barcode scanning successfully", async ({ page }) => {
    ...
    await expect(
      page.locator("text=Position the barcode inside the scan area"),
    ).toBeVisible();
    ...
    await expect(page.locator('[data-testid="applicator-serial"]')).toHaveValue(
      testApplicator.serialNumber,
    );
    await expect(page.locator('[data-testid="applicator-type"]')).toHaveValue(
      testApplicator.applicatorType,
    );
    await expect(page.locator('[data-testid="seed-quantity"]')).toHaveValue(
      testApplicator.seedQuantity.toString(),
    );

    // Verify success message
    await expect(
      page.locator("text=Applicator validated successfully"),
    ).toBeVisible();
  });

  test("should handle scanner initialization errors gracefully", async ({
    page,
  }) => {
    ...
    await expect(
      page.locator('[data-testid="manual-entry-fallback"]'),
    ).toBeVisible();
  });
  ...
  test("should support multiple barcode formats", async ({ page }) => {
    ...
    for (const barcode of barcodeFormats) {
      ...
      await expect(
        page.locator('[data-testid="applicator-serial"]'),
      ).toHaveValue(barcode.text);
      ...
      await expect(
        page.locator("text=Applicator processed successfully"),
      ).toBeVisible();
    }
  });

  test("should toggle between scanner and manual entry modes", async ({
    page,
  }) => {
    // Start in scanner mode
    await expect(page.locator('[data-testid="qr-reader"]')).toBeVisible();
    await expect(
      page.locator('[data-testid="manual-entry-form"]'),
    ).not.toBeVisible();
  ...
  test("should handle invalid barcode scanning", async ({ page }) => {
    ...
    for (const invalidBarcode of invalidBarcodes) {
      ...
      if (invalidBarcode.trim() === "") {
        await expect(
          page.locator("text=Invalid barcode: empty or whitespace"),
        ).toBeVisible();
  ...
  test("should handle rapid successive scans", async ({ page }) => {
    ...
    const processingMessages = await page
      .locator("text=Processing applicator")
      .count();
  ...
  test("should maintain scanner state during validation", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="scanner-loading"]'),
    ).not.toBeVisible();
    await expect(
      page.locator("text=Position the barcode inside the scan area"),
    ).toBeVisible();
  ...
  test("should support barcode scanning workflow integration", async ({
    page,
  }) => {
    ...
    for (const applicator of testData.applicators) {
      ...
      await expect(
        page.locator('[data-testid="applicator-serial"]'),
      ).toHaveValue(applicator.serialNumber);
      ...
      // Add removal comment
      await page.fill(
        '[data-testid="removal-comments"]',
        `Removed via barcode scan - ${applicator.serialNumber}`,
      );
      ...
      await WaitHelpers.waitForProgressUpdate(
        page,
        processedCount * applicator.seedQuantity,
      );
    }

    // Verify complete workflow
    const totalSeeds = testData.applicators.reduce(
      (sum, app) => sum + app.seedQuantity,
      0,
    );
    await expect(
      page.locator(`text=Removed: ${totalSeeds} / ${totalSeeds}`),
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="complete-treatment-button"]'),
    ).toBeEnabled();
  });
...
test.describe("Manual Entry Mode", () => {
  ...
  test("should validate manual entry with fuzzy matching", async ({ page }) => {
    ...
    const variations = [
      ...
      testApplicator.serialNumber.substring(
        0,
        testApplicator.serialNumber.length - 1,
    ...
    for (const variation of variations) {
      ...
      if (
        ...
        await expect(
          page.locator(`text=${testApplicator.serialNumber}`),
        ).toBeVisible();
      } else {
  ...
  test("should show applicator suggestions list", async ({ page }) => {
    ...
    for (const applicator of testData.applicators) {
      await expect(
        page.locator(`text=${applicator.serialNumber}`),
      ).toBeVisible();
      await expect(
        page.locator(`text=${applicator.seedQuantity} seeds`),
      ).toBeVisible();
    }
    ...
    await page.click(
      `[data-testid="select-applicator-${testData.applicators[0].serialNumber}"]`,
    );

    // Verify applicator details are filled
    await expect(page.locator('[data-testid="applicator-serial"]')).toHaveValue(
      testData.applicators[0].serialNumber,
    );
  });
