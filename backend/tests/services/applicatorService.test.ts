...
describe('Applicator Service', () => {
  ...
  describe('addApplicator', () => {
    test('should add applicator successfully', async () => {
      ...
      const result = await applicatorService.addApplicator(
        'test-treatment-uuid-001',
        {
          serialNumber: 'APP001-2025-001',
          usageType: 'full',
    ...
    test('should validate treatment ID format', async () => {
      await expect(
        applicatorService.addApplicator(
          'invalid-uuid',
          {
            serialNumber: 'APP001-2025-001',
            usageType: 'full'
      ...
      ).rejects.toThrow('Treatment ID must be a valid UUID');
    });
    ...
    test('should validate required fields', async () => {
      await expect(
        applicatorService.addApplicator(
          'test-treatment-uuid-001',
          {
            usageType: 'full' // Missing serialNumber
          },
          'test-user-uuid-001'
        )
      ...
      ).rejects.toThrow('Usage type is required');
    });

    test('should validate usage type values', async () => {
      await expect(
        applicatorService.addApplicator(
          'test-treatment-uuid-001',
          {
      ...
      ).rejects.toThrow('Invalid usage type. Must be "full", "faulty", or "none"');
    });

    test('should require comments for faulty applicators', async () => {
      await expect(
        applicatorService.addApplicator(
          'test-treatment-uuid-001',
          {
      ...
      ).rejects.toThrow('Comments are required for faulty applicators');
    });
    ...
    test('should continue with local save even if Priority save fails', async () => {
      const { Treatment, Applicator } = require('../../src/models');

      Treatment.findByPk.mockResolvedValue(mockTreatment);
      Applicator.create.mockResolvedValue(mockApplicator);

      // Mock Priority save failure
      jest.spyOn(applicatorService, 'saveApplicatorToPriority').mockResolvedValue({
        success: false,
        message: 'Priority API error'
      });
      ...
      expect(result.serialNumber).toBe('APP001-2025-001');
      expect(Applicator.create).toHaveBeenCalled();
    });
  });
