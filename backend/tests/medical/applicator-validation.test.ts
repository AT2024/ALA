  ...
  describe("State Transitions", () => {
    ...
    describe("SCANNED -> FAULTY scenarios", () => {
      test("should handle faulty applicator with partial seed insertion", () => {
        ...
        expect(faultyState.insertedSeedsQty).toBeLessThan(
          faultyState.seedQuantity,
        );
        expect(faultyState.faultyReason).toBeDefined();
      });

      test("should require faulty reason when marking as faulty", () => {
        const validateFaultyApplicator = (applicator: {
          usageType: string;
          faultyReason?: string;
        }) => {
        ...
        expect(() => validateFaultyApplicator({ usageType: "faulty" })).toThrow(
          "Faulty applicators must have a reason",
        );

        expect(
          validateFaultyApplicator({
            usageType: "faulty",
    ...
    describe("Invalid state transition prevention", () => {
      test("should prevent transition from COMPLETED back to SCANNED", () => {
        ...
        expect(() => validateStateTransition("COMPLETED", "SCANNED")).toThrow(
          "Invalid transition: COMPLETED -> SCANNED",
        );

        expect(() => validateStateTransition("FAULTY", "INSERTED")).toThrow(
          "Invalid transition: FAULTY -> INSERTED",
        );
      });
      ...
      test("should prevent skipping INSERTED state", () => {
        ...
        expect(() => validateStateTransition("SCANNED", "COMPLETED")).toThrow(
          "Cannot complete without insertion",
        );
      });
    });
  });
  ...
  describe("Medical Safety Checks", () => {
    ...
    describe("Previously used applicator detection", () => {
      test("should detect already scanned applicator in current treatment", () => {
        ...
        expect(validateNotAlreadyScanned("APP-001").scenario).toBe(
          "already_scanned",
        );
      ...
      test("should detect applicator used in previous treatment", () => {
        const previouslyUsedApplicators = [
          {
            serialNumber: "APP-OLD-001",
            usageType: "full",
            treatmentId: "TRT-001",
          },
          {
            serialNumber: "APP-OLD-002",
            usageType: "faulty",
            treatmentId: "TRT-001",
          },
        ];
