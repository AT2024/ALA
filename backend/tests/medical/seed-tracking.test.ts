  ...
  describe("Insertion Tracking", () => {
    describe("Total seeds calculation", () => {
      ...
      test("should handle mixed usage types correctly", () => {
        ...
        const totalInserted = applicators.reduce(
          (sum, a) => sum + a.insertedSeedsQty,
          0,
        );
      ...
      test("should calculate total available seeds from all scanned applicators", () => {
        ...
        const totalAvailable = scannedApplicators.reduce(
          (sum, a) => sum + a.seedQuantity,
          0,
        );
    ...
    describe("Partial use calculations", () => {
      test("should track remaining seeds after partial insertion", () => {
        ...
        const remainingSeeds =
          applicator.seedQuantity - applicator.insertedSeedsQty;
      ...
      test("should calculate insertion efficiency percentage", () => {
        ...
        const calculateEfficiency = (apps: typeof applicators) => {
          ...
          const totalCapacity = usedApplicators.reduce(
            (sum, a) => sum + a.seedQuantity,
            0,
          );
          const totalInserted = usedApplicators.reduce(
            (sum, a) => sum + a.insertedSeedsQty,
            0,
          );
  ...
  describe("Removal Tracking", () => {
    describe("Removal count accuracy", () => {
      test("should track seeds removed per applicator", () => {
        ...
        const totalRemoved = insertedApplicators.reduce(
          (sum, a) => sum + a.removedSeedsQty,
          0,
        );
        const totalInserted = insertedApplicators.reduce(
          (sum, a) => sum + a.insertedSeedsQty,
          0,
        );
      ...
      test("should calculate removal progress percentage", () => {
        ...
        const removalProgress = Math.round(
          ((treatment.removedSeeds + treatment.individualRemovedSeeds) /
            treatment.totalInsertedSeeds) *
            100,
        );
    ...
    describe("Discrepancy detection", () => {
      test("should detect seed count discrepancy during removal", () => {
        ...
        const discrepancy =
          applicator.insertedSeedsQty - applicator.removedSeedsQty;
      ...
      test("should track discrepancy reasons per applicator", () => {
        ...
        expect(applicatorsWithIssues[0].discrepancyReason).toBe(
      ...
      test("should calculate total discrepancy across treatment", () => {
        ...
        const totalInserted = removalData.applicators.reduce(
          (sum, a) => sum + a.insertedSeedsQty,
          0,
        );
        const applicatorRemoved = removalData.applicators.reduce(
          (sum, a) => sum + a.removedSeedsQty,
          0,
        );
        const totalRemoved =
          applicatorRemoved + removalData.individualSeedsRemoved;
    ...
    describe("Individual seed removal tracking", () => {
      test("should track seeds removed individually (not via applicator)", () => {
        ...
        const effectiveRemoved =
          treatmentRemoval.applicatorRemovedSeeds +
          treatmentRemoval.individualSeedsRemoved;

        expect(effectiveRemoved).toBe(58);
        expect(effectiveRemoved).toBeLessThanOrEqual(
          treatmentRemoval.totalInsertedSeeds,
        );
      });
      ...
      const auditLog = [
        {
          ...
          action: "APPLICATOR_SCANNED",
          seedCount: 25,
        },
        {
          timestamp: "2025-01-01T10:05:00Z",
          action: "SEEDS_INSERTED",
          seedCount: 25,
        },
        {
