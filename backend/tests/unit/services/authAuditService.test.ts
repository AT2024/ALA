...
describe("AuthAuditService", () => {
  ...
  describe("maskIdentifier", () => {
    it("should mask email addresses correctly", () => {
      expect(authAuditService.maskIdentifier("user@example.com")).toBe(
        "us***@example.com",
      );
      expect(authAuditService.maskIdentifier("ab@test.com")).toBe(
        "a***@test.com",
      );
    ...
    it("should mask phone numbers correctly", () => {
      expect(authAuditService.maskIdentifier("555-123-4567")).toBe(
        "555-***-4567",
      );
      expect(authAuditService.maskIdentifier("1234567890")).toBe(
        "123-***-7890",
      );
      expect(authAuditService.maskIdentifier("+1 (555) 123-4567")).toBe(
        "155-***-4567",
      );
    });
  ...
  describe("logLoginSuccess", () => {
    ...
    it("should not throw on database error", async () => {
      ...
      // Should not throw
      await expect(
        authAuditService.logLoginSuccess("user-123"),
  ...
  describe("logLogout", () => {
    ...
    it("should not throw on database error", async () => {
      ...
      await expect(
        authAuditService.logLogout("user-123"),
      ).resolves.not.toThrow();
      expect(logger.error).toHaveBeenCalled();
    });
  });
  ...
  describe("logSessionTimeout", () => {
    ...
    it("should not throw on database error", async () => {
      ...
      await expect(
        authAuditService.logSessionTimeout("user-123"),
      ).resolves.not.toThrow();
      expect(logger.error).toHaveBeenCalled();
    });
  });
