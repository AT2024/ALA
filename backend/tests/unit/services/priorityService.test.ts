...
describe("Priority Service", () => {
  ...
  describe("getUserSiteAccess", () => {
    test("should handle test@example.com with test data", async () => {
      ...
      const result =
        await priorityService.getUserSiteAccess("test@example.com");
    ...
    test("should handle regular user with Priority API", async () => {
      ...
      const result = await priorityService.getUserSiteAccess(
        "regular@hospital.com",
      );
    ...
    test("should handle Alpha Tau employee with position code 99", async () => {
      ...
      const result =
        await priorityService.getUserSiteAccess("admin@alphatau.com");
  ...
  describe("getUserFromPhonebook", () => {
    test("should find user by email", async () => {
      ...
      const result =
        await priorityService.getUserFromPhonebook("test@example.com");
  ...
  describe("getOrdersForSiteWithFilter", () => {
    test("should get orders for site with date filter", async () => {
      ...
      const result = await priorityService.getOrdersForSiteWithFilter(
        "100078",
        "real@user.com",
        "2025-07-10",
      );
    ...
    test("should throw error for real users when API fails", async () => {
      ...
      await expect(
        priorityService.getOrdersForSiteWithFilter(
          "100078",
          "real@user.com",
          "2025-07-10",
        ),
  ...
  describe("checkRemovalStatus", () => {
    test("should check removal status from Priority API", async () => {
      ...
      const result = await priorityService.checkRemovalStatus(
        "SO25000010",
        "real@user.com",
      );
  ...
  describe("Date filtering and OData queries", () => {
    test("should construct proper OData date filter", async () => {
      ...
      await priorityService.getOrdersForSiteWithFilter(
        "100078",
        "real@user.com",
        "2025-07-10",
      );
      ...
      expect(mockAxiosInstance.get).toHaveBeenCalledWith("/ORDERS", {
        params: {
          $filter: expect.stringMatching(
            /CUSTNAME eq '100078' and SIBD_TREATDAY ge 2025-07-10T00:00:00Z and SIBD_TREATDAY lt 2025-07-11T00:00:00Z/,
          ),
          $select:
            "ORDNAME,CUSTNAME,REFERENCE,CURDATE,SIBD_TREATDAY,ORDSTATUSDES,SBD_SEEDQTY,SBD_PREFACTIV,DETAILS,SIBD_SEEDLEN,SIBD_INDICATION",
        },
