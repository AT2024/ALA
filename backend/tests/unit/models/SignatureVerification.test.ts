...
describe("SignatureVerification Model", () => {
  ...
  describe("generateCode", () => {
    ...
    test("should set 1-hour expiration", async () => {
      ...
      const timeDiff = Math.abs(
        mockVerification.verificationExpires.getTime() -
  ...
  describe("Edge Cases", () => {
    test("should handle bcrypt hash error", async () => {
      mockedBcrypt.hash.mockRejectedValueOnce(
        new Error("Bcrypt hash error") as never,
      );

      await expect(mockVerification.generateCode()).rejects.toThrow(
        "Bcrypt hash error",
      );
    });

    test("should handle bcrypt compare error", async () => {
      mockedBcrypt.compare.mockRejectedValueOnce(
        new Error("Bcrypt compare error") as never,
      );

      await expect(mockVerification.verifyCode("123456")).rejects.toThrow(
