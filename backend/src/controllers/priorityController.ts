...
// @access  Public
export const validateUserEmail = asyncHandler(
...
// @access  Private
export const debugPriorityConnection = asyncHandler(
  async (req: Request, res: Response) => {
    try {
...
// @access  Private
export const getOrdersForSite = asyncHandler(
  async (req: Request, res: Response) => {
    try {
...
// @access  Private
export const getAllowedSitesForUser = asyncHandler(
  async (req: Request, res: Response) => {
    try {
...
// @access  Private
export const getOrdersForSiteAndDate = asyncHandler(
  async (req: Request, res: Response) => {
    try {
      ...
      logger.info(
        `Fetching orders for site: ${site}, date: ${date}, procedureType: ${procedureType}`,
      );
      ...
      logger.info(
        `Retrieved ${orders.length} orders from Priority service for site ${site}`,
      );
...
export const getOrderSubform = asyncHandler(
  async (req: Request, res: Response) => {
    try {
      ...
      logger.info(
        `Retrieved ${subformData.length} subform records for order ${orderId}`,
      );
...
export const validateApplicator = asyncHandler(
  async (req: Request, res: Response) => {
    try {
      ...
      logger.info(
        `Validating applicator: ${serialNumber} for site: ${currentSite}`,
      );

      // Validate applicator using Priority service
      const validationResult =
        await priorityService.validateApplicatorForManualEntry(
          serialNumber,
          currentSite,
          currentDate,
        );
      ...
      logger.info(
        `Applicator validation result for ${serialNumber}: ${validationResult.valid ? "VALID" : "INVALID"}`,
      );
...
// @access  Private
export const getAvailableApplicators = asyncHandler(
  async (req: Request, res: Response) => {
    try {
      const { site, currentDate } = req.query;

      logger.info(
        `Received request for available applicators - Site: ${site}, Date: ${currentDate}, User: ${req.user?.email}`,
      );

      if (!site || !currentDate) {
        logger.error(
          "Missing required parameters: site and currentDate are required",
        );
        res.status(400).json({
          success: false,
      ...
      if (!userHasFullAccess && !userSiteCodes.includes(site)) {
        logger.error(
          `User ${req.user?.email} does not have access to site ${site}. User sites: ${userSiteCodes.join(", ")}`,
        );
        res.status(403).json({
          success: false,
      ...
      logger.info(
        `User ${req.user?.email} has access to site ${site}. Getting available applicators...`,
      );
      ...
      logger.info(
        `Successfully retrieved ${applicators.length} available applicators for site ${site}`,
      );
      ...
      logger.error(
        `Error getting available applicators for site ${req.query.site}: ${error.message}`,
      );
...
// @access  Private
export const searchApplicators = asyncHandler(
  async (req: Request, res: Response) => {
    try {
      ...
      logger.info(
        `Searching applicators with query: "${query}" for site: ${site}`,
      );
      ...
      logger.info(
        `Search result - Found: ${searchResult.found}, Suggestions: ${searchResult.suggestions?.length || 0}`,
      );
...
// @access  Private
export const checkRemovalStatus = asyncHandler(
  async (req: Request, res: Response) => {
    try {
      const { orderId } = req.params;
      ...
      logger.error(
        `Error checking removal status for order ${req.params.orderId}: ${error.message}`,
      );
      res.status(500).json({
        success: false,
        error: error.message,
