...
import {
  formatAndEnrichApplicators,
  fetchSeedLength,
} from "../utils/applicatorFormatter";
...
async function getContinuationInfo(
  treatment: Treatment,
): Promise<ContinuationInfo | undefined> {
  ...
  if (!parentPdf) {
    logger.warn(
      `[CONTINUATION] Parent treatment ${treatment.parentTreatmentId} has no PDF`,
      {
  ...
  logger.info(
    `[CONTINUATION] Found parent PDF created at ${parentPdf.signedAt}`,
    {
      treatmentId: treatment.id,
      parentTreatmentId: treatment.parentTreatmentId,
    },
...
async function loadTestUserRemovalApplicators(
  ...
  if (!treatmentNumber) {
    logger.warn(
      `Test user removal but no treatment number found for treatment ${treatment.id}`,
    );
    return [];
  }
  ...
  try {
    ...
    if (testApplicators && testApplicators.length > 0) {
      ...
      logger.info(
        `Test user removal: loaded ${formattedApplicators.length} applicators from test data for treatment ${treatment.id}`,
      );
...
// @access  Private
export const getTreatmentById = asyncHandler(
  async (req: Request, res: Response) => {
...
// @access  Private
export const completeTreatment = asyncHandler(
  async (req: Request, res: Response) => {
    const treatment = await treatmentService.getTreatmentById(req.params.id);
    ...
    logger.info(
      `Completing treatment with ${orderIds.length} Priority order(s): ${orderIds.join(", ")}`,
    );
    ...
    if (
      ...
      (!treatment.priorityId || treatment.priorityId === treatment.id)
    ) {
      logger.info(
        `Skipping Priority update for removal treatment ${req.params.id} - no valid Priority order`,
      );
      ...
      try {
        ...
        for (const orderId of orderIds) {
          try {
            logger.info(
              `Updating Priority order ${orderId} to status ${status}`,
            );
            const statusResult =
              await applicatorService.updateTreatmentStatusInPriority(
                req.params.id,
                status,
            ...
            if (statusResult.success) {
              ...
              logger.warn(
                `Failed to update Priority order ${orderId}: ${statusResult.message}`,
              );
            }
          } catch (orderError: any) {
            failedOrders.push(orderId);
            logger.error(
              `Error updating Priority order ${orderId}:`,
              orderError,
            );
          }
        }
        ...
        if (updatedOrders.length === orderIds.length) {
          ...
          if (treatment.type === "insertion" && failedOrders.length > 0) {
            res.status(500);
            throw new Error(
              `Failed to update ${failedOrders.length} Priority order(s): ${failedOrders.join(", ")}`,
            );
          }
        } else {
        ...
        if (treatment.type === "removal") {
          logger.error(
            `Error updating Priority for removal ${req.params.id}:`,
            error,
          );
          priorityUpdateStatus =
            "Priority update failed (continuing with local completion)";
        } else {
    ...
    // Sync applicator usage data to Priority (non-blocking)
    try {
      const syncResult =
        await applicatorService.syncAllApplicatorsUsageToPriority(
          req.params.id,
        );
      logger.info(
        `Applicator usage sync for treatment ${req.params.id}: ${syncResult.synced} synced, ${syncResult.failed} failed, ${syncResult.skipped} skipped`,
      );
    } catch (syncError: any) {
      // Don't block treatment completion on sync failure
      logger.error(
        `Applicator usage sync failed for treatment ${req.params.id} (continuing): ${syncError.message}`,
      );
    }

    // Complete treatment locally
    const completedTreatment = await treatmentService.completeTreatment(
      req.params.id,
      req.user.id,
    );
...
// @access  Private
export const updateRemovalProcedure = asyncHandler(
  async (req: Request, res: Response) => {
    const treatment = await treatmentService.getTreatmentById(req.params.id);
    ...
    const updatedTreatment = await treatmentService.updateRemovalProcedure(
      req.params.id,
      req.body,
    );
...
// @access  Private
export const updateTreatmentStatus = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    if (!statusResult.success) {
      res.status(500);
      throw new Error(
        statusResult.message || "Failed to update treatment status in Priority",
      );
    }
...
// @access  Private
export const getTreatmentApplicators = asyncHandler(
  async (req: Request, res: Response) => {
    const treatment = await treatmentService.getTreatmentById(req.params.id);
    ...
    logger.info(
      `Getting applicators for treatment ${req.params.id}, type: ${treatment.type}, user: ${req.user.email}`,
    );
    ...
    if (treatment.isComplete) {
      logger.info(
        `Treatment ${req.params.id} is complete - fetching from local database to preserve status`,
      );
      const applicators = await applicatorService.getApplicators(
        req.params.id,
        treatment.type,
      );
    ...
    logger.info(
      `Treatment has ${orderIds.length} Priority order(s): ${orderIds.join(", ")}`,
    );
    ...
    if (orderIds.length > 1) {
      try {
        ...
        if (allApplicators.length > 0) {
          logger.info(
            `Total applicators from combined treatment: ${allApplicators.length}`,
          );

          // Use shared utility for enrichment and formatting
          const seedLength =
            orderIds.length > 0 ? await fetchSeedLength(orderIds[0]) : null;
          const formattedApplicators = await formatAndEnrichApplicators(
            allApplicators,
            {
        ...
        logger.error(
          `Error fetching applicators for combined treatment: ${error}`,
        );
    ...
    if (orderIds.length === 1 && req.user.email !== config.testUserEmail) {
      try {
        logger.info(
          `Single order treatment - fetching from Priority API for order ${orderIds[0]}`,
        );
        ...
        if (orderApplicators && orderApplicators.length > 0) {
          logger.info(
            `Found ${orderApplicators.length} applicators from Priority for single order`,
          );
    ...
    const treatmentNumber =
      req.user.email === config.testUserEmail && treatment.type === "removal"
    ...
    if (
      ...
      logger.info(
        `Test user removal treatment - loading from test data for order ${treatmentNumber}`,
      );
      ...
      try {
        ...
        const orderIdsToFetch =
          orderIds.length > 0 ? orderIds : [treatmentNumber];
        ...
        if (allApplicators.length > 0) {
          logger.info(
            `Found ${allApplicators.length} applicators from test data (${orderIds.length} order(s))`,
          );
...
// @access  Private
export const addApplicator = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      logger.info(
        `Applicator ${applicator.serialNumber} added to treatment ${treatmentId}`,
      );
      ...
      logger.error(
        `Failed to add applicator to treatment ${treatmentId}: ${error.message}`,
      );
      ...
      if (error.message === "Treatment not found") {
        ...
        error.message === "Cannot add applicator to a completed treatment"
      ) {
        res.status(400);
...
// @access  Private
export const getRemovalCandidates = asyncHandler(
  async (req: Request, res: Response) => {
      ...
      try {
        ...
        if (removalCandidates.length > 0) {
          ...
          const treatmentDate = new Date(
            candidate.SIBD_TREATDAY || candidate.CURDATE,
          );
          ...
          const candidateResponse = {
            ...
            patientName:
              ...
              candidate.DETAILS ||
              candidate.PNAME ||
              "Test Patient", // Use patientName from mapping or fallback to DETAILS/PNAME
            ...
            activity:
              (candidate.SBD_SEEDQTY || 0) * (candidate.SBD_PREFACTIV || 0), // Calculate total activity
    ...
    try {
      ...
      if (treatmentNumber) {
        const treatment = await treatmentService.getTreatmentBySubjectId(
          treatmentNumber as string,
          site as string,
        );
        ...
        // Serialize to avoid circular references from Sequelize associations
        const treatmentJson =
          typeof treatment.toJSON === "function"
            ? treatment.toJSON()
            : treatment;
        ...
        res.status(200).json({
          ...
          treatment: {
            ...
            eligibilityReasons: {
              daysSinceInsertion,
              validDayRange:
                daysSinceInsertion >= 13 && daysSinceInsertion <= 21,
              isInsertion,
              isCompleted,
            },
          },
        ...
        const eligibleTreatments = await treatmentService.getRemovalCandidates(
          site as string,
          req.user?.id || "",
        );
...
// @access  Private
export const exportTreatment = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    if (format === "csv") {
      // Generate CSV
      let csv =
        "Serial Number,Seed Quantity,Usage Type,Insertion Time,Comments,Is Removed\n";
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename=treatment-${treatment.id}.csv`,
      );
      res.send(csv);
    } else if (format === "pdf") {
      ...
      logger.info(
        `PDF downloaded for treatment ${treatment.id} by user ${req.user?.id}`,
      );

      res.setHeader("Content-Type", "application/pdf");
      res.setHeader(
        "Content-Disposition",
        `attachment; filename=treatment-${treatment.id}.pdf`,
      );
...
// @access  Private
export const initializeFinalization = asyncHandler(
  async (req: Request, res: Response) => {
    const treatment = await treatmentService.getTreatmentById(req.params.id);
    ...
    const existingPdf = await TreatmentPdf.findOne({
      where: { treatmentId: treatment.id },
    });
    if (existingPdf) {
      res.status(400).json({
    ...
    if (isAlphaTau) {
      ...
      res.status(200).json({
        ...
        signerPosition:
          req.user.metadata?.positionCode?.toString() || "hospital_staff",
        treatmentId: treatment.id,
      });
    }
  },
...
export const getSiteUsersForFinalization = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      const formattedUsers = siteUsers
        .map((user: any) => ({
      ...
      logger.error(
        `Error getting site users for finalization: ${error.message}`,
      );
      res.status(500);
...
export const sendFinalizationCode = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    if (!isAlphaTauAdmin(req.user)) {
      res.status(403);
      throw new Error(
        "Only Alpha Tau administrators can send verification codes",
      );
    }
    ...
    try {
      ...
      logger.info(
        `Verification code sent to ${targetEmail} for treatment ${treatment.id}`,
      );
    } catch (emailError: any) {
      logger.warn(`Failed to send verification email: ${emailError.message}`);
      // Continue anyway - code is logged in dev mode
...
// @access  Private (Position 99 only)
export const verifyAndFinalize = asyncHandler(
  async (req: Request, res: Response) => {
...
export const autoFinalize = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    const {
      signerName: requestSignerName,
      signerPosition: requestSignerPosition,
      availableApplicators,
    } = req.body;
    ...
    const signerPosition =
      requestSignerPosition || positionCode?.toString() || "hospital_staff";
 ...
 * @access  Private
 */
export const checkContinuable = asyncHandler(
  async (req: Request, res: Response) => {
    const treatmentId = req.params.id;

    logger.info(
      `[TREATMENT_CONTINUATION] Checking continuability for treatment ${treatmentId}`,
      {
        userId: req.user?.id,
        treatmentId,
      },
    ...
    const eligibility =
      await treatmentService.checkContinuationEligibility(treatmentId);
 ...
 * @access  Private
 */
export const createContinuation = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    logger.info(
      `[TREATMENT_CONTINUATION] Creating continuation for treatment ${parentTreatmentId}`,
      {
        userId,
        parentTreatmentId,
      },
    ...
    const eligibility =
      await treatmentService.checkContinuationEligibility(parentTreatmentId);
    ...
    const continuation = await treatmentService.createContinuationTreatment(
      parentTreatmentId,
      userId,
    );
 ...
 * @access  Private
 */
export const getContinuations = asyncHandler(
  async (req: Request, res: Response) => {
    const treatmentId = req.params.id;

    logger.info(
      `[TREATMENT_CONTINUATION] Getting continuations for treatment ${treatmentId}`,
      {
        userId: req.user?.id,
        treatmentId,
      },
 ...
 * @access  Private
 */
export const getParentTreatment = asyncHandler(
  async (req: Request, res: Response) => {
    const treatmentId = req.params.id;

    logger.info(
      `[TREATMENT_CONTINUATION] Getting parent treatment for ${treatmentId}`,
      {
        userId: req.user?.id,
        treatmentId,
      },
    ...
    const parentTreatment =
      await treatmentService.getParentTreatment(treatmentId);
