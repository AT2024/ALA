...
export async function formatAndEnrichApplicators(
  ...
  const formattedApplicators = await Promise.all(
    applicators.map(async (app: any) => {
      ...
      if (!catalog && app.PARTDES) {
        try {
          catalog = await priorityService.getPartNameFromDescription(
            app.PARTDES,
          );
        } catch (e: any) {
          logger.warn(
            `Catalog lookup failed for ${app.SERNUM} with PARTDES "${app.PARTDES}": ${e.message}`,
          );
        }
      }
      ...
      return {
        id:
          app.SIBD_REPPRODPAL ||
          `${priorityIdPrefix || "unknown"}-${app.SERNUM}`,
  ...
  if (missingCatalog > 0 || !seedLength) {
    logger.warn(
      `Enrichment: ${missingCatalog} applicators without catalog, seedLength=${seedLength}`,
    );
  }
 ...
 */
export async function fetchSeedLength(
  orderName: string,
): Promise<number | null> {
  try {
