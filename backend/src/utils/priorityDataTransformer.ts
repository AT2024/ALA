...
export const transformPriorityApplicatorData = (
  ...
  logger.debug(`${logPrefix} Starting Priority data transformation`, {
    ...
    rawDataTypes: Object.fromEntries(
      Object.entries(rawData || {}).map(([k, v]) => [k, typeof v]),
    ),
  });
  ...
  try {
    ...
    if (rawData.serialNumber) {
      if (typeof rawData.serialNumber === "string") {
        ...
        warnings.push(
          `Serial number converted from ${typeof rawData.serialNumber} to string`,
        );
      }
    } else {
    ...
    if (rawData.usageType) {
      ...
      if (mappedUsageType) {
        ...
        if (normalizedUsageType !== mappedUsageType) {
          warnings.push(
            `Usage type mapped from '${rawData.usageType}' to '${mappedUsageType}'`,
          );
        }
      } else {
        errors.push(
          `Invalid usage type: '${rawData.usageType}'. Valid values: ${Object.keys(usageTypeMapping).join(", ")}`,
        );
      }
    } else {
    ...
    if (rawData.insertionTime) {
      if (rawData.insertionTime instanceof Date) {
        ...
        if (!isNaN(parsedDate.getTime())) {
          ...
          warnings.push(
            `Invalid insertion time format: '${rawData.insertionTime}', using current time`,
          );
          transformedData.insertionTime = new Date();
        }
      } else {
        warnings.push(
          `Insertion time is not a valid date type: ${typeof rawData.insertionTime}, using current time`,
        );
        transformedData.insertionTime = new Date();
      }
    } else {
    ...
    if (rawData.seedQuantity !== undefined) {
      if (typeof rawData.seedQuantity === "number") {
        ...
        if (!isNaN(parsedQuantity)) {
          ...
          warnings.push(
            `Seed quantity converted from ${typeof rawData.seedQuantity} to number`,
          );
        } else {
          transformedData.seedQuantity = 0;
          warnings.push(
            `Invalid seed quantity: '${rawData.seedQuantity}', using 0`,
          );
        }
      }
    } else {
    ...
    if (rawData.insertedSeedsQty !== undefined) {
      if (typeof rawData.insertedSeedsQty === "number") {
        transformedData.insertedSeedsQty = Math.max(
          0,
          rawData.insertedSeedsQty,
        );
      } else {
        ...
        if (!isNaN(parsedQuantity)) {
          ...
          warnings.push(
            `Inserted seeds quantity converted from ${typeof rawData.insertedSeedsQty} to number`,
          );
        } else {
          transformedData.insertedSeedsQty = 0;
          warnings.push(
            `Invalid inserted seeds quantity: '${rawData.insertedSeedsQty}', using 0`,
          );
        }
      }
    } else {
      ...
      warnings.push(
        "Inserted seeds quantity not provided, using seed quantity value",
      );
    }
    ...
    if (rawData.seedLength !== undefined && rawData.seedLength !== null) {
      if (typeof rawData.seedLength === "number") {
        ...
        if (!isNaN(parsedLength)) {
          ...
          warnings.push(
            `Seed length converted from ${typeof rawData.seedLength} to number`,
          );
        }
      }
    }
...
export const validatePriorityDataStructure = (
  ...
  if (data.serialNumber && typeof data.serialNumber !== "string") {
    issues.push(
      `Serial number should be string, got ${typeof data.serialNumber}`,
    );
  }

  if (data.seedQuantity != null && typeof data.seedQuantity !== "number") {
    issues.push(
      `Seed quantity should be number, got ${typeof data.seedQuantity}`,
    );
  }
