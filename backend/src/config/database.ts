...
function getDatabaseUrl(): string {
  return (
    ...
    "postgres://postgres:postgres@localhost:5432/ala_db"
  );
}
...
const canAttemptConnection = (): boolean => {
  ...
  if (circuitBreaker.state === "OPEN") {
    ...
    const timeSinceLastFailure =
      now.getTime() - (circuitBreaker.lastFailure?.getTime() || 0);
...
export const initializeDatabase = async (
  ...
  while (retryCount < maxRetries) {
    ...
    try {
      if (retryCount > 0) {
        logger.info(
          `Retrying database connection (${retryCount}/${maxRetries}) after ${delay}ms delay...`,
        );
      ...
      logger.error(
        `Database connection error (attempt ${retryCount}/${maxRetries}): ${errorMessage}`,
      );
      ...
      if (retryCount >= maxRetries) {
        ...
        if (process.env.NODE_ENV === "production") {
          logger.error(
            "Server will exit as database is required in production",
          );
          process.exit(1);
        } else {
          logger.warn(
