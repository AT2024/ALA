...
export const config = {
  ...
  // Priority API
  priorityApiTimeout: parseInt(
    process.env.PRIORITY_API_TIMEOUT_MS || "30000",
    10,
  ),
  priorityApiShortTimeout: parseInt(
    process.env.PRIORITY_API_SHORT_TIMEOUT_MS || "10000",
    10,
  ),
  ...
  slowResponseThresholdMs: parseInt(
    process.env.SLOW_RESPONSE_THRESHOLD_MS || "3000",
    10,
  ),

  // Database health
  dbHealthCacheMs: parseInt(process.env.DB_HEALTH_CACHE_MS || "300000", 10),
  dbHealthThresholdMs: parseInt(
    process.env.DB_HEALTH_THRESHOLD_MS || "5000",
    10,
  ),
  dbReconnectIntervalMs: parseInt(
  ...
  // Email configuration
  emailSenderAddress:
    process.env.AZURE_EMAIL_SENDER_ADDRESS || "DoNotReply@alphatau.com",
  ...
  testUserUuid:
    process.env.TEST_USER_UUID || "47605b24-e71b-479b-93c5-8b7ce1c17098",
};
