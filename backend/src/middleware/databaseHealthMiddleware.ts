...
class DatabaseHealthCache {
  ...
  public async getHealthStatus(): Promise<DatabaseHealthResult> {
    ...
    if (
      ...
      now - new Date(this.lastHealthCheck.timestamp).getTime() <
...
export const databaseHealthCheck = async (
  ...
  const requestId =
    req.requestId || `health_${Math.random().toString(36).substr(2, 9)}`;
  ...
  try {
    ...
    logger.warn(
      `[DB_HEALTH] Health check failed, allowing request to proceed`,
      {
        requestId,
        method: req.method,
        url: req.originalUrl,
 ...
 */
export const criticalOperationHealthCheck = async (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
  const requestId =
    req.requestId || `critical_${Math.random().toString(36).substr(2, 9)}`;
  ...
  try {
    ...
    if (!healthStatus.isHealthy) {
      logger.error(
        `[DB_HEALTH] Database unhealthy, blocking critical operation`,
        {
          requestId,
          method: req.method,
          url: req.originalUrl,
      ...
      return res.status(503).json({
        ...
        error: "Service Unavailable",
        message:
          "Database is currently unavailable for critical operations. Please try again later.",
        details: {
          operation: "critical_database_operation",
          healthCheck: {
            timestamp: healthStatus.timestamp,
    ...
    if (healthStatus.connectionTime && healthStatus.connectionTime > 2000) {
      ...
      if (healthStatus.connectionTime > config.dbHealthThresholdMs) {
        logger.error(
          `[DB_HEALTH] Database latency too high, blocking critical operation`,
          {
            requestId,
            connectionTime: `${healthStatus.connectionTime}ms`,
        ...
        return res.status(503).json({
          ...
          error: "Service Unavailable",
          message:
            "Database response time is too high for critical operations.",
          details: {
            connectionTime: `${healthStatus.connectionTime}ms`,
 ...
 */
export const getDatabaseHealthStatus =
  async (): Promise<DatabaseHealthResult> => {
