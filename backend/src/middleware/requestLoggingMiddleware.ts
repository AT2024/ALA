 ...
 */
export const treatmentRequestLoggingMiddleware = (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
  ...
  // Preserve existing requestId from general middleware, use treatment-specific for treatment logs
  if (!req.requestId) {
    req.requestId = requestId;
  }
      ...
      responseData:
        statusCode >= 400 || process.env.NODE_ENV === "development"
          ...
          : {
              ...
              recordCount: Array.isArray(responseData)
                ? responseData.length
                : undefined,
            },
      timestamp: new Date().toISOString(),
