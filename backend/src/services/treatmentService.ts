...
export const treatmentService = {
    ...
    try {
      ...
      if (userId) {
        ...
        if (user && user.role !== "admin") {
          ...
          userFilter = {
            [Op.or]: [{ userId }, { site: { [Op.in]: userSites } }],
          };
        }
      }
      ...
      // For removal treatments, apply the 14-21 day window rule only if we have insertion treatments
      if (params.type === "removal") {
        const today = new Date();
        logger.info(
          `Applying removal treatment filter for ${dbTreatments.length} treatments`,
        );
        ...
        const filteredDbTreatments = dbTreatments.filter((treatment) => {
          ...
          const insertionTreatment = dbTreatments.find(
            (t) =>
              t.type === "insertion" &&
          ...
          if (!insertionTreatment) {
            logger.debug(
              `No insertion treatment found for removal treatment ${treatment.id}`,
            );
            return false;
          }
          ...
          logger.debug(
            `Treatment ${treatment.id}: ${daysSinceInsertion} days since insertion, in window: ${isInWindow}`,
          );
        ...
        logger.info(
          `Removal filter: ${dbTreatments.length} -> ${filteredDbTreatments.length} treatments`,
        );
      ...
      if (userSites.length > 0) {
        const priorityTreatments = await priorityService.getTreatmentsForSites(
          userSites,
          params,
        );
  ...
  async getTreatmentById(id: string, transaction?: any) {
    ...
    try {
      ...
      if (!id || typeof id !== "string") {
        logger.error(
          `[TREATMENT_SERVICE] [${queryId}] Invalid treatment ID format`,
          {
            id,
            idType: typeof id,
            idValue: id,
      ...
      logger.debug(
        `[TREATMENT_SERVICE] [${queryId}] Checking database connection`,
      );
      try {
        await sequelize.authenticate();
        logger.debug(
          `[TREATMENT_SERVICE] [${queryId}] Database connection verified`,
        );
      } catch (dbError: any) {
        logger.error(
          `[TREATMENT_SERVICE] [${queryId}] Database connection failed`,
          {
      ...
      logger.debug(
        `[TREATMENT_SERVICE] [${queryId}] Executing database query`,
        {
          treatmentId: id,
          queryOptions: {
      ...
      if (queryDuration > 1000) {
        logger.warn(
          `[TREATMENT_SERVICE] [${queryId}] Slow database query detected`,
          {
            treatmentId: id,
            queryDuration: `${queryDuration}ms`,
            threshold: "1000ms",
            recommendation:
              "Consider optimizing query or checking database performance",
          },
        );
      }
      ...
      if (!treatment) {
        ...
        logger.error(
          `[TREATMENT_SERVICE] [${queryId}] Treatment not found in database`,
          {
            treatmentId: id,
            queryDuration: `${queryDuration}ms`,
        ...
        // Diagnostic query: only run in development to avoid extra DB round-trip in production
        if (transaction && process.env.NODE_ENV === "development") {
          try {
            logger.debug(
              `[TREATMENT_SERVICE] [${queryId}] Checking treatment existence without transaction`,
            );
            const treatmentWithoutTransaction = await Treatment.findByPk(id);
            logger.debug(
              `[TREATMENT_SERVICE] [${queryId}] Treatment exists without transaction`,
              {
                found: !!treatmentWithoutTransaction,
                possibleTransactionIsolationIssue:
                  !!treatmentWithoutTransaction,
              },
            );
          } catch (debugError: any) {
            logger.debug(
              `[TREATMENT_SERVICE] [${queryId}] Debug query also failed`,
              {
      ...
      logger.info(
        `[TREATMENT_SERVICE] [${queryId}] Treatment successfully retrieved`,
        {
          treatmentId: id,
          queryDuration: `${queryDuration}ms`,
      ...
      logger.error(
        `[TREATMENT_SERVICE] [${queryId}] Error in getTreatmentById`,
        {
          queryId,
          treatmentId: id,
          hasTransaction: !!transaction,
  ...
  async createTreatment(data: any, userId: string) {
    try {
      ...
      if (data._priorityIds && Array.isArray(data._priorityIds)) {
        ...
        logger.info(
          `Creating combined pancreas treatment with ${data._priorityIds.length} orders: ${priorityId}`,
        );
      }
    ...
    try {
      ...
      if (treatment.type === "removal") {
        ...
        if (totalSeeds !== removedSeeds) {
          logger.warn(
            `Treatment ${id} completed with missing seeds: ${totalSeeds - removedSeeds} seeds unaccounted for`,
          );
        }
      }
  ...
  async updateRemovalProcedure(
    ...
    try {
      ...
      await treatment.update({
        ...
        additionalRemovalDate: data.additionalRemovalDate
          ? new Date(data.additionalRemovalDate)
          : undefined,
        reasonNotSameDate: data.reasonNotSameDate || undefined,
        discrepancyClarification: data.discrepancyClarification
          ? JSON.stringify(data.discrepancyClarification)
          : undefined,
        individualSeedsRemoved: data.individualSeedsRemoved || 0,
        individualSeedNotes: data.individualSeedNotes
          ? JSON.stringify(data.individualSeedNotes)
          : undefined,
        ...
        groupComments: data.groupComments
          ? JSON.stringify(data.groupComments)
          : undefined,
        individualSeedComment: data.individualSeedComment || undefined,
      });

      logger.info(
        `Updated removal procedure data for treatment ${treatmentId}`,
      );
  ...
  async validateTreatmentTimeWindow(
    treatmentId: string,
    applicatorId?: string,
  ) {
    try {
      ...
      if (treatment.type === "insertion" && daysSinceTreatment === 1) {
        ...
        if (user && (user.role === "hospital" || user.role === "alphatau")) {
          return {
            valid: true,
            message:
              "Hospital staff and Alpha Tau personnel can edit the day after insertion",
          };
        }
      }
  ...
  async getTreatmentBySubjectId(subjectId: string, site: string) {
    ...
    try {
      ...
      if (!subjectId || typeof subjectId !== "string") {
        logger.error(
          `[TREATMENT_SERVICE] [${queryId}] Invalid subjectId format`,
          {
            subjectId,
      ...
      logger.error(
        `[TREATMENT_SERVICE] [${queryId}] Error in getTreatmentBySubjectId`,
        {
          queryId,
          subjectId,
          site,
  ...
  async getRemovalCandidates(site: string, userId: string) {
    ...
    try {
      ...
      const user = await User.findByPk(userId);
      if (!user) {
        logger.error(`[TREATMENT_SERVICE] [${queryId}] User not found`, {
          userId,
        });
      ...
      const isAdmin =
        user.role === "admin" || Number(user.metadata?.positionCode) === 99;

      // Check if user has access to the site
      if (!isAdmin && !userSiteCodes.includes(site)) {
        logger.error(
          `[TREATMENT_SERVICE] [${queryId}] User does not have access to site`,
          {
            userId,
            site,
            userSiteCodes,
      ...
      logger.debug(
        `[TREATMENT_SERVICE] [${queryId}] Calculating removal window`,
        {
      ...
      logger.debug(
        `[TREATMENT_SERVICE] [${queryId}] Executing removal candidates query`,
        {
          site,
          userId,
      ...
      logger.info(
        `[TREATMENT_SERVICE] [${queryId}] Removal candidates query completed`,
        {
          site,
          userId,
          queryDuration: `${queryDuration}ms`,
      ...
      if (queryDuration > 1000) {
        logger.warn(
          `[TREATMENT_SERVICE] [${queryId}] Slow removal candidates query detected`,
          {
            site,
            userId,
            queryDuration: `${queryDuration}ms`,
      ...
      logger.error(
        `[TREATMENT_SERVICE] [${queryId}] Error in getRemovalCandidates`,
        {
          queryId,
          site,
          userId,
  ...
  async checkContinuationEligibility(
    treatmentId: string,
    ...
    try {
      ...
      if (!treatment) {
        logger.warn(`[TREATMENT_SERVICE] [${queryId}] Treatment not found`, {
          treatmentId,
        });
      ...
      if (!treatment.isComplete) {
        logger.info(`[TREATMENT_SERVICE] [${queryId}] Treatment not complete`, {
          treatmentId,
        });
        return {
          canContinue: false,
          reason: "Treatment must be completed before continuation",
        };
      }
      ...
      if (treatment.type !== "insertion") {
        logger.info(
          `[TREATMENT_SERVICE] [${queryId}] Not an insertion treatment`,
          {
            treatmentId,
            type: treatment.type,
          },
        ...
        return {
          canContinue: false,
          reason: "Only insertion treatments can be continued",
        };
      }

      // Check 24-hour window from lastActivityAt (or completedAt as fallback)
      const lastActivity =
        treatment.lastActivityAt ||
      ...
      const reusableApplicators =
        treatment.applicators?.filter((app) =>
          reusableStatuses.includes(app.status || ""),
        ) || [];
      ...
      logger.error(
        `[TREATMENT_SERVICE] [${queryId}] Error in checkContinuationEligibility`,
        {
          queryId,
          treatmentId,
          error: {
  ...
  async createContinuationTreatment(
    ...
    try {
      ...
      const eligibility =
        await this.checkContinuationEligibility(parentTreatmentId);
      ...
      logger.info(
        `[TREATMENT_SERVICE] [${queryId}] Continuation treatment created`,
        {
      ...
      logger.error(
        `[TREATMENT_SERVICE] [${queryId}] Error in createContinuationTreatment`,
        {
          queryId,
          parentTreatmentId,
          userId,
  ...
  async getContinuations(treatmentId: string): Promise<Treatment[]> {
    ...
    try {
      ...
      logger.error(
        `[TREATMENT_SERVICE] [${queryId}] Error in getContinuations`,
        {
          queryId,
          treatmentId,
          error: {
  ...
  async getParentTreatment(treatmentId: string): Promise<Treatment | null> {
    ...
    try {
      ...
      const parentTreatment = await Treatment.findByPk(
      ...
      logger.error(
        `[TREATMENT_SERVICE] [${queryId}] Error in getParentTreatment`,
        {
          queryId,
          treatmentId,
          error: {
