...
export type ApplicatorStatus =
  (typeof APPLICATOR_STATUSES)[keyof typeof APPLICATOR_STATUSES];

/**
 * Array of all valid status values (for validation)
 */
export const ALL_STATUSES: ApplicatorStatus[] =
  Object.values(APPLICATOR_STATUSES);
 ...
 */
export const PANC_PROS_TRANSITIONS: Record<
  ApplicatorStatus,
  ApplicatorStatus[]
> = {
...
export const LIST_ITEM_COLORS: Record<ApplicatorStatus, string> = {
  ...
  [APPLICATOR_STATUSES.FAULTY]:
    "bg-gray-900 text-white border-l-4 border-l-gray-900",
  [APPLICATOR_STATUSES.DISPOSED]:
    "bg-gray-900 text-white border-l-4 border-l-gray-900",
  [APPLICATOR_STATUSES.DISCHARGED]:
    "bg-gray-900 text-white border-l-4 border-l-gray-900",
 ...
 */
export const isInProgressStatus = (
  status: ApplicatorStatus | string | null | undefined,
): boolean => {
 ...
 */
export const requiresAdminForConflict = (
  status: ApplicatorStatus | string | null | undefined,
): boolean => {
  if (!status) return false;
...
export const getStatusColors = (
  ...
  return (
    STATUS_COLORS[status as ApplicatorStatus] ||
    STATUS_COLORS[APPLICATOR_STATUSES.SEALED]
  );
};
 ...
 */
export const getListItemColor = (
  status: ApplicatorStatus | string | null | undefined,
): string => {
  if (!status) return LIST_ITEM_COLORS[APPLICATOR_STATUSES.SEALED];
  return (
    LIST_ITEM_COLORS[status as ApplicatorStatus] ||
